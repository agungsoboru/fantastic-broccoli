...
ag=False # aggressive mode flag
        ...
        # Baca kata-kata tambahan dari file txt
with open("dns.txt", "r") as file:
    tambahan_list = [line.strip() for line in file.readlines()]
...
class ThreadedUDPRequestHandler(SocketServer.BaseRequestHandler):
    ...
    def handle(self):
        ...
        # if ag and ("windows" in query or query in blockList): #wildcard domain
        #     block=True
        # elif not ag and query in blockList:
        #     block=True

        # if ag and (("windows" in query or query in blockList) or any(keyword in query for keyword in ["edge", "agung"])):
        #     block = True
        # elif not ag and query in blockList: #di tabahkan oleh chatgpt untuk support banyak keyword
        #     block = True




        # Check apakah kata terdapat dalam query
        if ag and (("windows" in query or query in blockList) or any(keyword in query for keyword in tambahan_list)):
            block = True
        elif not ag and query in blockList:
            block = True
...
'''
    ...
    while True:
        print("file dns.txt dan BlockList.txt harus 1 folder")
        print("Use Aggressive Mode (Blocks all urls with keyword \"windows\") [y/n]",end=' : ')
        ag= input()[0]
        if ag in "yY":
            print(Fore.GREEN+"\n** AGGRESSIVE MODE ON **\n")
            ag=True
